...
function formatCheckedAt(value: string | null) {
  ...
  return Number.isNaN(date.getTime()) ? "Recently checked" : `Updated ${new Intl.DateTimeFormat("en-US", { timeZone: "UTC", hour: "2-digit", minute: "2-digit", hour12: false }).format(date)} UTC`;
}
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-end">
            ...
            <div className="relative rounded-[2rem] border border-white/15 bg-white/[.08] p-5 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-6">
              ...
              {payload?.fallback && <p className="mt-4 text-xs leading-5 text-amber-200">{payload.keyless ? "Preview mode: this snapshot comes from CoinGecko’s shared public endpoint because no server provider key is configured. It is not a production polling setup." : "The primary market provider was unavailable, so this snapshot comes from the configured fallback. Check the timestamp before comparing it with another venue."}</p>}
            </div>
          </div>
        </div>
      </section>
