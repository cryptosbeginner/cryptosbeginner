...
function normalizeCoin(raw: unknown, index: number): MarketCoin {
  ...
  const change1h = asNumber(coin.priceChange1h ?? coin.price_change_percentage_1h);
  const change7d = asNumber(coin.priceChange7d ?? coin.price_change_percentage_7d);
  const change30d = asNumber(coin.priceChange1m ?? coin.price_change_percentage_30d);
...
async function fetchCoinGecko(limit: number, currency: MarketCurrency) {
  ...
  if (!apiKey) throw new Error("CoinGecko credentials are not configured");
...
export async function getMarketData(limit: number, currency: MarketCurrency): Promise<MarketResponse> {
  ...
  try {
    ...
    try {
      return { provider: "CoinGecko", currency, checkedAt, staleAfterSeconds: 90, data: await fetchCoinGecko(limit, currency), fallback: true };
    } catch {
      return { provider: "unavailable", currency, checkedAt, staleAfterSeconds: 90, data: [], error: "Market data is temporarily unavailable." };
    }
  }
}
