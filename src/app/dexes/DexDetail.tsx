  ...
  return (
    <main className="bg-white">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:px-8 lg:py-14">
        <div><p className="text-xs font-black uppercase tracking-[0.2em] text-indigo-700">{service.categories.slice(0, 3).join(" · ")}</p><h1 className="mt-3 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">{service.name}</h1><p className="mt-4 max-w-2xl text-xl leading-8 text-slate-600">{service.shortDescription}</p><div className="mt-6 flex flex-wrap gap-2"><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">{service.access}</span><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">Reviewed {service.reviewedAt}</span>{service.isKycConditional && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">Policy checks may apply</span>}</div><div className="mt-8 flex flex-wrap gap-3"><a href={service.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-indigo-700 px-5 py-3 text-sm font-black text-white transition hover:bg-indigo-800">Visit official site <Glyph name="external" /></a>{service.feeSourceUrl && <a href={service.feeSourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-5 py-3 text-sm font-black text-slate-800 transition hover:border-indigo-400">Fee documentation <Glyph name="external" /></a>}</div></div>
        <HeroImage service={service} />
      </section>

      <section className="mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8"><div className="grid gap-4 md:grid-cols-4"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Glyph name="wallet" /><p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Access model</p><p className="mt-1 font-bold text-slate-950">{service.access}</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Glyph name="fee" /><p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Fee lens</p><p className="mt-1 font-bold text-slate-950">{service.fees.split(".")[0]}.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Glyph name="network" /><p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Network scope</p><p className="mt-1 font-bold text-slate-950">{service.chains.slice(0, 2).join(", ")}</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Glyph name="shield" /><p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Verification</p><p className="mt-1 font-bold text-slate-950">{service.isNoKycCandidate && !service.isKycConditional ? "Provider-stated no-ID flow" : service.isKycConditional ? "Conditional" : "Product terms"}</p></div></div></section>

      <section className="bg-slate-950 text-white"><div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-lime-300">Editorial read</p><h2 className="mt-3 text-3xl font-black">What stands out about {service.name}?</h2><p className="mt-5 text-base leading-8 text-slate-300">{service.editorialSummary}</p><div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-xs font-black uppercase tracking-wide text-slate-400">Best suited to</p><p className="mt-2 text-lg font-bold text-white">{service.bestFor}</p></div></div><div className="rounded-3xl border border-white/10 bg-white/5 p-6"><h2 className="text-xl font-black">Read before you trade</h2><div className="mt-5 flex gap-3 text-slate-300"><Glyph name="warning" /><p className="text-sm leading-6">{service.caution}</p></div><div className="mt-6 space-y-4 text-sm leading-6 text-slate-300"><p><span className="font-bold text-white">Availability:</span> {service.availability}</p><p><span className="font-bold text-white">Verification:</span> {service.verification}</p><p><span className="font-bold text-white">Assets:</span> {service.assets}</p></div></div></div></section>
