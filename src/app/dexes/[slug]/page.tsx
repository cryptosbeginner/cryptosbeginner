...
export function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  return params.then(({ slug }) => {
    ...
    if (!service || !service.isDex) return {};
    const title = `${service.name}: Fees, Networks & Risks | CryptosBeginner`;
    const description = `${service.name} profile: ${service.shortDescription} Compare fees, networks, access model, verification, pros, cons and official sources.`;
...
export default async function DexDetailRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    "@graph": [
      { "@type": "WebPage", "@id": `${canonical}#webpage`, url: canonical, name: `${service.name}: Fees, Networks & Risks`, description: service.shortDescription, image: { "@id": `${canonical}#image` }, dateModified: service.reviewedAt, isPartOf: { "@id": `${siteUrl}/#website` } },
      { "@type": "ImageObject", "@id": `${canonical}#image`, url: service.imageUrl, contentUrl: service.imageUrl, caption: service.imageAlt },
      { "@type": "BreadcrumbList", "@id": `${canonical}#breadcrumbs`, itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: siteUrl }, { "@type": "ListItem", position: 2, name: "DEXes", item: `${siteUrl}/dexes` }, { "@type": "ListItem", position: 3, name: service.name, item: canonical }] },
